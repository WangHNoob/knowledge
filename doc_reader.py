...
def _read_cache(filepath, force=False):
    """读取已转换的 md 文件。文件新于源文件时返回文本，否则返回 None。"""
    if force:
        return None
    ext = os.path.splitext(filepath)[1].lower()
    if ext not in _CACHED_EXTS:
        return None
    md_path = _get_cache_path(filepath)
...
def _write_cache(filepath, text):
    ...
    if ext not in _CACHED_EXTS or not text:
        return
    # 修复 openpyxl 在中文环境下保存时 escape 下划线的问题 (RES\_001 -> RES_001)
...
def _convert_table_to_text(table_lines):
    """将markdown表格转换为纯文本格式。
    ...
    for row in data_rows:
        row_text = " | ".join(f"{headers[i]}: {row[i]}" if headers and i < len(headers) else row[i]
...
def _filter_xlsx_noise(text):
    """过滤 xlsx 转 markdown 后的噪音行，并将表格转换为纯文本。

    保留: sheet 标题(##)、数据行转纯文本
    删除: 纯 NaN 行、纯数字/ID 行、空行、表头分隔线
    """
    lines = text.split('\n')
    filtered = []
    table_buffer = []
    ...
    for line in lines:
        ...
        if stripped.startswith('## '):
            # 转换之前的表格
            if table_buffer:
                converted = _convert_table_to_text(table_buffer)
                filtered.extend(converted)
                table_buffer = []
        ...
        if stripped.startswith('|'):
            ...
            continue
        # 含中文的非表格行 -> 可能是描述文本，保留
        if '一' <= stripped[0] <= '鿿' if stripped else False:
            # 转换之前的表格
            if table_buffer:
                converted = _convert_table_to_text(table_buffer)
                filtered.extend(converted)
...
def scan_dir(directory, extensions=None):
    ...
    for root, dirs, filenames in os.walk(directory):
        # 跳过缓存目录和隐藏目录
        dirs[:] = [d for d in dirs if d != '.cache' and not d.startswith('.')]
