...
从 wiki/_tables/groups.json 读取所有表族名，
通过 LLM 将英文表族名翻译为中文，生成 cn_en_map.json。

Usage:
    uv run python processed/tools/build_cn_en_map.py
    uv run python processed/tools/build_cn_en_map.py --force   # 强制重新生成
    uv run python processed/tools/build_cn_en_map.py --dry-run # 只看 LLM 输出，不写文件
"""
...
def _load_groups() -> list[str]:
    groups_path = Path(_ROOT) / "wiki" / "_tables" / "groups.json"
...
def main() -> None:
    ...
    out_path = Path(_ROOT) / "wiki" / "_tables" / "cn_en_map.json"
